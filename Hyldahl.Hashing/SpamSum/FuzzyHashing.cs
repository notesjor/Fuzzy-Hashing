...
namespace Hyldahl.Hashing.SpamSum
{
  public class FuzzyHashing
  {
    ...
    public static SpamSumSignature Calculate(Stream stream)
    {
      ...
      var ctx = new SpamSumContext();

      var filepos = stream.Position;
    ...
    private static int has_common_substring(byte[] s1, byte[] s2)
    {
      int i, j;
      int num_hashes;
      var hashes = new uint[SPAMSUM_LENGTH];
      ...
      /* first compute the windowed rolling hash at each offset in
         the first string */
      for (i = 0; i < s1.Length; i++) hashes[i] = roll_hash(roll_state, s1[i]);
      num_hashes = i;
      ...
      {
        var h = roll_hash(roll_state, s2[i]);
    ...
    private static void ss_init(SpamSumContext ctx, Stream stream)
    {
      ...
      while (ctx.block_size * SPAMSUM_LENGTH < ctx.total_chars) ctx.block_size = ctx.block_size * 2;
    }
